...
- **demo**: the hub section of the navigation page demonstrates the new API — snap and RTL toggles, first/previous/next/last navigation through `scrollToSection`, a live in-view section readout bound to `selectionchanged`, and mixed-width sections
...
- **hub**: programmatic scroll API — read-only `sections` list, a `selectedIndex` accessor (reads the section whose start is nearest the scroll position; the setter scrolls that section to the container gutter and, unlike `metro-pivot`, stores no state), and `scrollToSection(index, behavior = "auto")`. `behavior: "smooth"` is opt-in and animates with the Metro easing and duration tokens instead of the browser curve; every programmatic scroll is instant under `prefers-reduced-motion: reduce`. RTL works through rect math, so the same call lands the section on the gutter under `dir="rtl"`.
...
- **hub-section**: `role="group"` labelled by its `header` attribute. The header keeps its pointer/hover styling (the Windows 8.1 hub header was clickable) but stays non-interactive at the component level — apps attach their own click listeners on the light-DOM section elements.
