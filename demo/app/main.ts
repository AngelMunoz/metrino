...
import { subscribe, getCurrentRoute, getRouteHandler, type RouterState } from "./router";
...
const routes: RouteDef[] = [
  { path: "/", title: "Home", handler: () => html`<metrino-home></metrino-home>` },
  {
    path: "/buttons",
    title: "Buttons",
...
export class MetrinoApp extends LitElement {
  ...
  constructor() {
    super();
    this.routeContent = html`<div style="padding: 24px; text-align: center;">Loading...</div>`;
  }
  ...
  render() {
    return html` <metrino-app-shell> ${this.routeContent} </metrino-app-shell> `;
  }
}
