...
import { baseTypography, scrollbarVisible, groupHeader } from "../../../src/styles/shared.ts";
...
function generateNames(count: number): { name: string; category: string }[] {
  const names = [
    ...
    "Uma", "Victor", "Wendy", "Xavier", "Yara", "Zack", "Adam", "Bella", "Chris", "Daisy"
  ];
...
function generateAlphabeticalNames(count: number): { name: string; category: string }[] {
  const names = [
    "Alice", "Aaron", "Adam", "Abigail", "Alexander",
    "Bella", "Bob", "Benjamin", "Brooke", "Brian",
    "Charlie", "Charlotte", "Chris", "Catherine", "Caleb",
    "Diana", "David", "Daniel", "Daisy", "Derek",
    "Eve", "Ethan", "Emma", "Edward", "Eleanor",
    "Frank", "Fiona", "Felix", "Faith", "Frederick",
    "Grace", "George", "Gemma", "Gabriel", "Georgia"
  ];
  const result: { name: string; category: string }[] = [];
  for (let i = 0; i < count; i++) {
...
function generateGridItems(count: number): { name: string; color: string }[] {
  const colors = ["#0078d4", "#e51400", "#fa6800", "#00a300", "#a200ff", "#d80073"];
...
const FLIP_VIEW_ITEMS = [
  { content: html`<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); height: 100%; display: flex; align-items: center; justify-content: center; color: white; font-size: 32px;">Welcome to Metro</div>` },
  { content: html`<div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); height: 100%; display: flex; align-items: center; justify-content: center; color: white; font-size: 32px;">Beautiful Design</div>` },
  { content: html`<div style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); height: 100%; display: flex; align-items: center; justify-content: center; color: white; font-size: 32px;">Smooth Animations</div>` },
  { content: html`<div style="background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%); height: 100%; display: flex; align-items: center; justify-content: center; color: white; font-size: 32px;">Touch Optimized</div>` }
];
...
export class MetrinoSelectionPage extends LitElement {
  ...
  #handleListViewSelection(e: CustomEvent): void {
    ...
    this.renderedListViewCount = lv?.shadowRoot?.querySelectorAll(".list-item").length || 0;
  ...
  #handleGridViewSelection(e: CustomEvent): void {
    ...
    this.renderedGridViewCount = gv?.shadowRoot?.querySelectorAll(".grid-item").length || 0;
  ...
  #handleLongListSelection(e: CustomEvent): void {
    ...
    this.renderedLongListCount = ll?.shadowRoot?.querySelectorAll(".list-item").length || 0;
  ...
  #showListPicker(): void {
    const picker = this.shadowRoot?.querySelector("metro-list-picker") as HTMLElement & { show: () => void };
    picker?.show();
  }
  ...
  render() {
    return html`
      ...
      <p>
        Metro-style selection components with virtualization, cross-slide gestures, semantic zoom, and jump list navigation.
        All virtualized components render only visible items for optimal performance.
      </p>

      <div class="section">
        <h2>metro-list-box</h2>
        <p>Classic list box with single, multiple, and extended selection modes. Use Ctrl+Click for extended mode.</p>
        
        <div class="controls">
          <metro-button @click=${() => { this.listBoxMode = "single"; }}>
            Single
          </metro-button>
          <metro-button @click=${() => { this.listBoxMode = "multiple"; }}>
            Multiple
          </metro-button>
          <metro-button @click=${() => { this.listBoxMode = "extended"; }}>
            Extended
          </metro-button>
        </div>
        ...
        <div class="demo-grid">
          <div class="demo-card">
            <h3>Selection Mode: ${this.listBoxMode}<span class="badge">${this.listBoxMode}</span></h3>
            <metro-list-box
              class="list-box-demo"
              selection-mode=${this.listBoxMode}
              @selectionchanged=${this.#handleListBoxSelection}
            >
              <div class="list-item" data-value="item1">Item 1 - First Option</div>
              <div class="list-item" data-value="item2">Item 2 - Second Option</div>
              <div class="list-item" data-value="item3">Item 3 - Third Option</div>
              <div class="list-item" data-value="item4">Item 4 - Fourth Option</div>
              <div class="list-item" data-value="item5">Item 5 - Fifth Option</div>
              <div class="list-item" data-value="item6">Item 6 - Sixth Option</div>
            </metro-list-box>
          ...
          <div class="demo-card">
            <h3>Features</h3>
            <ul style="color: var(--metro-foreground-secondary, rgba(255, 255, 255, 0.7)); margin: 0; padding-left: 20px; line-height: 1.8;">
      ...
      <div class="section">
        ...
        <p>Virtualized list view with 1,000+ items. Only visible items are rendered (check stats below). Supports grouping, cross-slide gestures, and keyboard navigation.</p>
        
        <div class="controls">
          <metro-button @click=${() => { this.listViewMode = "none"; }}>
            No Selection
          </metro-button>
          <metro-button @click=${() => { this.listViewMode = "single"; }}>
            Single
          </metro-button>
          <metro-button @click=${() => { this.listViewMode = "multiple"; }}>
            Multiple
          </metro-button>
          <metro-button @click=${() => { this.listViewMode = "extended"; }}>
            Extended
          </metro-button>
        </div>
        ...
        <div class="stats">
          Total Items: 1,000 | Rendered Items: ~${this.renderedListViewCount} | Selection: ${this.listViewSelection}
        </div>
      </div>

      <div class="section">
        <h2>metro-grid-view</h2>
        <p>Virtualized grid view with 500 items. Supports responsive columns, grouping, and selection with cross-slide gestures.</p>
        
        <div class="controls">
          <metro-button @click=${() => { this.gridViewMode = "none"; }}>
            No Selection
          </metro-button>
          <metro-button @click=${() => { this.gridViewMode = "single"; }}>
            Single
          </metro-button>
          <metro-button @click=${() => { this.gridViewMode = "multiple"; }}>
            Multiple
          </metro-button>
          <metro-button @click=${() => { this.gridViewMode = "extended"; }}>
            Extended
          </metro-button>
        </div>
        ...
        <div class="stats">
          Total Items: 500 | Rendered Items: ~${this.renderedGridViewCount} | Selection: ${this.gridViewSelection}
        </div>
      </div>

      <div class="section">
        <h2>metro-flip-view</h2>
        <p>Swipeable carousel with touch gestures and navigation controls. Drag or use arrow buttons to navigate.</p>
      ...
      <div class="section">
        ...
        <p>Hierarchical tree view with expand/collapse functionality. Click folders to expand, use keyboard arrows to navigate.</p>
        ...
        <div class="selection-info">Selected Node: ${this.treeSelected || "None"}</div>
      </div>

      <div class="section">
        <h2>metro-long-list-selector</h2>
        <p>Virtualized long list with 10,000 items grouped alphabetically. Features jump list sidebar for quick navigation. Try scrolling and clicking letters on the right.</p>
        
        <div class="controls">
          <metro-button @click=${() => { this.longListMode = "none"; }}>
            No Selection
          </metro-button>
          <metro-button @click=${() => { this.longListMode = "single"; }}>
            Single
          </metro-button>
          <metro-button @click=${() => { this.longListMode = "multiple"; }}>
            Multiple
          </metro-button>
        </div>
        ...
        <div class="stats">
          Total Items: 10,000 | Rendered Items: ~${this.renderedLongListCount} | Selection: ${this.longListSelection}
        </div>
      </div>

      <div class="section">
        <h2>metro-semantic-zoom</h2>
        <p>Zoom between two views using pinch gesture or Ctrl+Scroll. Click the button or use touch gestures to toggle views.</p>
      ...
      <div class="section">
        ...
        <p>Modal picker dialog for selecting items from a list. Click the button to open the picker.</p>
      ...
      <div class="section">
        ...
        <div class="demo-card">
          ...
          <p>All list components use windowing/virtualization to render only visible items:</p>
          <ul style="color: var(--metro-foreground-secondary, rgba(255, 255, 255, 0.7)); margin: 0; padding-left: 20px; line-height: 1.8;">
            <li><strong>metro-list-view:</strong> 1,000 items → ~15 items rendered at any time</li>
            <li><strong>metro-grid-view:</strong> 500 items → ~20-30 items rendered depending on viewport</li>
            <li><strong>metro-long-list-selector:</strong> 10,000 items → ~15 items rendered with jump list navigation</li>
          </ul>
        </div>
      </div>
    `;
