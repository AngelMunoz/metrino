...
export class MetrinoDialogsPage extends LitElement {
  ...
  render() {
    return html`
      ...
      <section class="section">
        ...
        <p class="description">
          A modal dialog with customizable content area and button slot.
          Supports light dismiss (click outside to close) and animated
          open/close with turnstile effect.
        </p>
        <div class="demo-row">
          <metro-button @click=${this.#openContentDialog}
            >Open Content Dialog</metro-button
          >
        </div>
        <metro-content-dialog id="content-dialog" title="Content Dialog">
          <div class="dialog-content">
            <p>
              This is a content dialog with a slot for any content you want to
              display.
            </p>
            <p>You can put forms, lists, or any other components here.</p>
          </div>
          <div slot="buttons" class="dialog-buttons">
            <metro-button @click=${this.#closeContentDialog}
              >Cancel</metro-button
            >
            <metro-button accent @click=${this.#closeContentDialog}
              >OK</metro-button
            >
          </div>
        </metro-content-dialog>
        <api-docs component="MetroContentDialog"></api-docs>
      </section>

      <section class="section">
        <h2>metro-message-dialog</h2>
        <p class="description">
          A simple alert dialog for displaying messages to the user. Click
          outside to dismiss.
        </p>
        <div class="demo-row">
          <metro-button @click=${this.#openMessageDialog}
            >Show Message Dialog</metro-button
          >
        </div>
        <metro-message-dialog id="message-dialog" title="Information">
          <p>
            This is a simple message dialog. It's useful for alerts,
            confirmations, and brief messages.
          </p>
          <div slot="buttons" class="dialog-buttons">
            <metro-button accent @click=${this.#closeMessageDialog}
              >OK</metro-button
            >
          </div>
        </metro-message-dialog>
        <api-docs component="MetroMessageDialog"></api-docs>
      </section>

      <section class="section">
        <h2>metro-flyout</h2>
        <p class="description">
          A contextual popup positioned relative to a target element. Supports
          different placements (top, bottom, left, right) and light dismiss.
        </p>
        <div class="demo-row">
          <metro-button
            id="flyout-top-btn"
            @click=${(e: Event) => this.#openFlyout(e, "top")}
            >Flyout Top</metro-button
          >
          <metro-button
            id="flyout-bottom-btn"
            @click=${(e: Event) => this.#openFlyout(e, "bottom")}
            >Flyout Bottom</metro-button
          >
          <metro-button
            id="flyout-left-btn"
            @click=${(e: Event) => this.#openFlyout(e, "left")}
            >Flyout Left</metro-button
          >
          <metro-button
            id="flyout-right-btn"
            @click=${(e: Event) => this.#openFlyout(e, "right")}
            >Flyout Right</metro-button
          >
        </div>
        <metro-flyout id="demo-flyout" placement="bottom">
          <div class="flyout-content">
            <h4>Flyout Title</h4>
            <p>
              This is a flyout with contextual content. Click outside to close.
            </p>
          </div>
        </metro-flyout>
        <api-docs component="MetroFlyout"></api-docs>
      </section>

      <section class="section">
        <h2>metro-settings-flyout</h2>
        <p class="description">
          A side panel that slides in from the right. Useful for settings,
          forms, or detailed content.
        </p>
        <div class="demo-row">
          <metro-button @click=${this.#openSettingsFlyout}
            >Open Settings</metro-button
          >
          <metro-button @click=${this.#openWideSettingsFlyout}
            >Open Wide Settings</metro-button
          >
        </div>
        ...
        <api-docs component="MetroSettingsFlyout"></api-docs>
      </section>
      ...
      <section class="section">
        ...
        <div class="demo-row">
          <metro-button @click=${() => this.#showToast("informational")}
            >Info Toast</metro-button
          >
          <metro-button @click=${() => this.#showToast("success")}
            >Success Toast</metro-button
          >
          <metro-button @click=${() => this.#showToast("warning")}
            >Warning Toast</metro-button
          >
          <metro-button @click=${() => this.#showToast("error")}
            >Error Toast</metro-button
          >
        </div>
        <div class="demo-row">
      ...
      <section class="section">
        ...
        <p class="description">
          A dropdown menu that appears below a button. Useful for context menus
          and action lists.
        </p>
        <div class="demo-row">
          <metro-button id="menu-flyout-btn" @click=${this.#openMenuFlyout}
            >Open Menu</metro-button
          >
        </div>
        ...
        <api-docs component="MetroMenuFlyout"></api-docs>
      </section>
    `;
  }
  ...
  #openFlyout(e: Event, placement: string): void {
    const flyout = this.shadowRoot?.querySelector(
      "#demo-flyout",
    ) as HTMLElement & {
